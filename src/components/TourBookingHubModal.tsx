...
export const TourBookingHubModal: React.FC<TourBookingHubModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'location' | 'chat' | 'guide'>('overview');
  ...
  const currentStatus = (selectedBookingId && localStatuses[selectedBookingId]) || bookingInList?.status || 'matched';
  const selectedBooking: TourBooking | null = bookingInList ? { ...bookingInList, status: currentStatus } : null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-5 bg-slate-950/80 backdrop-blur-md animate-fade-in overflow-y-auto">
      <div className="bg-white rounded-3xl max-w-3xl w-full h-[90vh] max-h-[90vh] min-h-[480px] overflow-hidden shadow-2xl border border-slate-200 flex flex-col relative my-auto">
        ...
        <div className="bg-amber-50 border-b border-amber-200/80 px-4 sm:px-5 py-2.5 sm:py-3 flex flex-wrap items-center justify-between gap-2.5 text-xs shrink-0">
          ...
          <div className="flex items-center space-x-2">
            <span className={`px-2.5 py-1 rounded-xl text-[10px] font-bold border ${
              selectedBooking.travelerConfirmedCompletion ? 'bg-emerald-100 text-emerald-900 border-emerald-300' : 'bg-white text-slate-600 border-slate-300'
            }`}>
              Traveler: {selectedBooking.travelerConfirmedCompletion ? '✓ Accepted' : '⏳ Pending'}
            </span>
            <span className={`px-2.5 py-1 rounded-xl text-[10px] font-bold border ${
              selectedBooking.guideConfirmedCompletion ? 'bg-emerald-100 text-emerald-900 border-emerald-300' : 'bg-white text-slate-600 border-slate-300'
            }`}>
        ...
        <div className="flex-1 min-h-0 flex flex-col md:flex-row overflow-hidden">
            ...
            <div className="space-y-1">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 md:flex md:flex-col gap-1.5">
                ...
                >
                  ...
                  <span className="truncate">{language === 'vi' ? '4. HDV & Hỗ Trợ' : '4. Guide Info'}</span>
                </button>
              </div>
            </div>
              ...
              <div className="space-y-1 text-[11px]">
                <button
                  ...
                  className={`w-full text-left p-1.5 rounded-xl font-bold flex items-center justify-between transition-all cursor-pointer ${
                    selectedBooking.status === 'matched' ? 'bg-teal-100 text-teal-950 font-extrabold border border-teal-300' : 'text-slate-500 hover:bg-slate-100'
                  }`}
                >
                  ...
                  {selectedBooking.status === 'matched' && <span className="text-[9px] bg-teal-800 text-white px-1.5 py-0.5 rounded-md">ACTIVE</span>}
                </button>
                ...
                <button
                  ...
                  className={`w-full text-left p-1.5 rounded-xl font-bold flex items-center justify-between transition-all cursor-pointer ${
                    selectedBooking.status === 'en_route' ? 'bg-amber-100 text-amber-950 font-extrabold border border-amber-300' : 'text-slate-500 hover:bg-slate-100'
                  }`}
                >
                  <span className="flex items-center space-x-1.5">
                    <span>🛵</span>
                  ...
                  {selectedBooking.status === 'en_route' && <span className="text-[9px] bg-amber-800 text-white px-1.5 py-0.5 rounded-md">ACTIVE</span>}
                </button>
                ...
                <button
                  ...
                  className={`w-full text-left p-1.5 rounded-xl font-bold flex items-center justify-between transition-all cursor-pointer ${
                    selectedBooking.status === 'in_progress' ? 'bg-emerald-100 text-emerald-950 font-extrabold border border-emerald-300' : 'text-slate-500 hover:bg-slate-100'
                  }`}
                >
                  <span className="flex items-center space-x-1.5">
                    <span>🎒</span>
                  ...
                  {selectedBooking.status === 'in_progress' && <span className="text-[9px] bg-emerald-800 text-white px-1.5 py-0.5 rounded-md">ACTIVE</span>}
                </button>
                ...
                <button
                  ...
                  className={`w-full text-left p-1.5 rounded-xl font-bold flex items-center justify-between transition-all cursor-pointer ${
                    selectedBooking.status === 'completed' ? 'bg-indigo-100 text-indigo-950 font-extrabold border border-indigo-300' : 'text-slate-500 hover:bg-slate-100'
                  }`}
                >
                  <span className="flex items-center space-x-1.5">
                    <span>🏁</span>
                  ...
                  {selectedBooking.status === 'completed' && <span className="text-[9px] bg-indigo-800 text-white px-1.5 py-0.5 rounded-md">ACTIVE</span>}
                </button>
              </div>
            </div>
          ...
          {/* Main Content Pane */}
          <div className="flex-1 min-h-0 p-4 sm:p-5 overflow-y-auto space-y-5 bg-slate-50/40">
            ...
            {activeTab === 'overview' && (
              <div className="space-y-4 animate-fade-in">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <button
                      ...
                      className={`p-2.5 rounded-2xl border text-xs font-bold transition-all cursor-pointer flex flex-col items-center justify-center text-center space-y-1 ${
                        selectedBooking.status === 'matched'
                    ...
                    >
                      ...
                      <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-extrabold ${selectedBooking.status === 'matched' ? 'bg-teal-900 text-teal-200' : 'text-slate-400'}`}>
                        {selectedBooking.status === 'matched' ? '● CURRENT' : 'Scheduled'}
                      </span>
                    </button>
                    ...
                    >
                      ...
                      <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-extrabold ${selectedBooking.status === 'en_route' ? 'bg-amber-900 text-amber-100' : 'text-slate-400'}`}>
                        {selectedBooking.status === 'en_route' ? '● CURRENT' : 'Departing'}
                      </span>
                    </button>
                    ...
                    >
                      ...
                      <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-extrabold ${selectedBooking.status === 'in_progress' ? 'bg-emerald-900 text-emerald-100' : 'text-slate-400'}`}>
                        {selectedBooking.status === 'in_progress' ? '● CURRENT' : 'Active'}
                      </span>
                    </button>
                    ...
                    >
                      ...
                      <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-extrabold ${selectedBooking.status === 'completed' ? 'bg-indigo-950 text-indigo-200' : 'text-slate-400'}`}>
                        {selectedBooking.status === 'completed' ? '● CURRENT' : 'Finished'}
                      </span>
                    </button>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-3xl bg-gradient-to-br from-slate-900 to-teal-950 text-white shadow-md space-y-3 relative overflow-hidden">
              ...
              <div className="space-y-4 animate-fade-in">
                ...
                <div className="p-3.5 rounded-2xl bg-teal-50 border border-teal-200 flex items-center justify-between text-xs">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <div>
                      <p className="font-bold text-slate-900">{selectedBooking.guideName}</p>
                      <p className="text-[10px] text-teal-700 font-semibold">{selectedBooking.guidePhone || '+84 908 123 456'}</p>
                    </div>
                  </div>

                  <span className="px-2.5 py-1 rounded-full bg-emerald-600 text-white font-extrabold text-[10px]">
                    ● Online Guide
                  </span>
                </div>
                ...
                <div className="border border-slate-200 rounded-3xl p-4 bg-white shadow-2xs space-y-3 flex flex-col justify-between min-h-[280px]">
                  ...
                  <div className="space-y-3 overflow-y-auto max-h-[240px] pr-1">
                    {messages.length === 0 ? (
                      <div className="text-center py-8 text-slate-400 text-xs space-y-1">
                        ...
                        <p className="text-[10px]">Send a quick message to coordinate pickup or timing!</p>
                      </div>
                    ) : (
                      messages.map((m) => (
                        <div
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-2 border-t border-slate-100 text-[11px]">
                    ...
                    <button
                      type="button"
                      onClick={() => handleSendMessage(undefined, "Should I bring anything specific for today?")}
                      className="px-2.5 py-1 rounded-xl bg-slate-100 hover:bg-teal-50 hover:text-teal-800 text-slate-700 font-medium transition-colors cursor-pointer border border-slate-200"
                    >
                      "What to bring?"
                    </button>
        ...
        <div className="p-3 sm:p-4 bg-white border-t border-slate-200 flex items-center justify-between shrink-0">
          <button
            ...
            className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold transition-all cursor-pointer flex items-center space-x-1.5"
          >
            <span className="material-symbols-outlined text-sm">print</span>
            <span>{language === 'vi' ? 'In / Lưu Thẻ Tour Pass PDF' : 'Print / Save Tour Pass PDF'}</span>
          </button>
